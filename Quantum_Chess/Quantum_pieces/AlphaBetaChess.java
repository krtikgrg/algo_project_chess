        ...
        for(int i=0;i<moveList.length();i+=5){
            ...
            else
            {
                if(alpha>=beta){
                   ...
                   // System.out.print(x1+" , "+y1+"---"+chessBoard[x1][y1]+"\n");
                    ...
                    switch (tomove){
                        case 0: moveList = possibleP(x1*8+y1);
                                break;
                        case 1: moveList = possibleK(x1*8+y1);
                                break;
                        case 2: moveList = possibleB(x1*8+y1);
                                break;
                        case 3: moveList = possibleR(x1*8+y1);
                                break;
                        case 4: moveList = possibleQ(x1*8+y1);
                                break;
                        case 5: moveList = possibleA(x1*8+y1);
                                break;
                    }
                    if(moveList.length()==0)
                    {
                        move = move.substring(0,2)+move.substring(0,2);
                        return move;
                    }
...
/*if(depth==0 && moveList.length()!=0)
  {	//limiting cases if searching depth becomes 0 or no possible moves are left (eg. checkmate)
//move+(Rating.rating(moveList.length(),depth)*(player*2-1));
int x1=Character.getNumericValue(move.charAt(0));
int y1=Character.getNumericValue(move.charAt(1));
System.out.print(x1+" , "+y1+"---"+chessBoard[x1][y1]+"\n");
Random rand = new Random();
