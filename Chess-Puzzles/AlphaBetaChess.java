...
public class AlphaBetaChess {
	...
	*/
	...
	public static void main(String[] args) {
		...
		playerColor=1;
		/*JOptionPane.showOptionDialog(null, "Do you want to play the first move?", 
	...
	public static void diceroll()
	{
		Random r = new Random();
			int a[] = {0,0,0};
			a[0] = r.nextInt()%6; a[1] = (r.nextInt()+a[0]*a[0])%6;
			a[2] = (r.nextInt()+a[0]*a[1]+a[1]*a[1]+a[0]*a[0])%6;
			for(int i=0;i<3;i++)
			{
				if(a[i]==0)
					die[i]="P";
	...
	public static String alphaBeta2(int depth , int beta , int alpha , String move , int player)
	{
		String moveList="";
		...
		moveList = sortMoves(moveList);
		move  = moveList.substring(0,5);
		return move ;
	}
	...
	public static String possibleMoves2(){
		String moveList="";
		for(int j=0;j<3;j++)
		{
            ...
            if(die[j].equals("A"))
            {
            	for(int i=0;i<64;i++)
            	{
            		if(chessBoard[i/8][i%8].equals("A"))
            			moveList+=possibleA(i);
            	}
            }
		}
		return moveList;
